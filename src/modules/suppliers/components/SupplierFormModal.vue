<script setup lang="ts">
// Crear/editar proveedor - formulario chico, mismo patron modal que
// CategoryFormModal/IngredientFormModal (resetForm/watch-on-open/
// fieldErrors/formError/submit).
import { computed, ref, watch } from 'vue'

import { useSystemAlert } from '@/composables/useSystemAlert'
import type { Supplier } from '@/types/supplier'
import { NxButton, NxInput, NxModal, NxTextarea } from '@/ui'
import { extractErrorMessage } from '@/utils/extractErrorMessage'
import { extractFieldErrors } from '@/utils/extractFieldErrors'

import { useSupplierMutations } from '../composables/useSupplierMutations'

const props = defineProps<{
  modelValue: boolean
  supplier: Supplier | null
}>()

const emit = defineEmits<{ 'update:modelValue': [value: boolean] }>()

const { createMutation, updateMutation } = useSupplierMutations()
const { notify } = useSystemAlert()

const name = ref('')
const taxId = ref('')
const phone = ref('')
const address = ref('')
const notes = ref('')
const fieldErrors = ref<Record<string, string>>({})
const formError = ref<string | null>(null)

function resetForm(): void {
  name.value = props.supplier?.name ?? ''
  taxId.value = props.supplier?.tax_id ?? ''
  phone.value = props.supplier?.phone ?? ''
  address.value = props.supplier?.address ?? ''
  notes.value = props.supplier?.notes ?? ''
  fieldErrors.value = {}
  formError.value = null
}

watch(
  () => props.modelValue,
  (open) => {
    if (open) {
      resetForm()
    }
  },
)

const isEdit = computed(() => props.supplier !== null)
const isSaving = computed(() => createMutation.isPending.value || updateMutation.isPending.value)
const modalTitle = computed(() => (isEdit.value ? 'Editar proveedor' : 'Nuevo proveedor'))

async function submit(): Promise<void> {
  fieldErrors.value = {}
  formError.value = null

  if (!name.value.trim()) {
    fieldErrors.value.name = 'El nombre es obligatorio.'
    return
  }

  const payload = {
    name: name.value.trim(),
    tax_id: taxId.value.trim() || null,
    phone: phone.value.trim() || null,
    address: address.value.trim() || null,
    notes: notes.value.trim() || null,
  }

  try {
    if (props.supplier) {
      await updateMutation.mutateAsync({ id: props.supplier.id, payload })
      notify('Proveedor actualizado')
    } else {
      await createMutation.mutateAsync(payload)
      notify('Proveedor creado')
    }
    emit('update:modelValue', false)
  } catch (error) {
    const fields = extractFieldErrors(error)
    if (Object.keys(fields).length > 0) {
      fieldErrors.value = fields
    } else {
      formError.value = extractErrorMessage(error, 'No pudimos guardar el proveedor.')
    }
  }
}
</script>

<template>
  <NxModal
    :model-value="modelValue"
    :title="modalTitle"
    size="md"
    @update:model-value="emit('update:modelValue', $event)"
  >
    <div class="flex flex-col gap-3">
      <p v-if="formError" class="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700">{{ formError }}</p>

      <NxInput v-model="name" label="Nombre" required :error="fieldErrors.name" />
      <NxInput v-model="taxId" label="NIT / documento (opcional)" :error="fieldErrors.tax_id" />
      <NxInput v-model="phone" label="Teléfono (opcional)" :error="fieldErrors.phone" />
      <NxInput v-model="address" label="Dirección (opcional)" :error="fieldErrors.address" />
      <NxTextarea v-model="notes" label="Notas (opcional)" :rows="2" />
    </div>

    <template #footer>
      <div class="flex gap-2">
        <NxButton variant="outline" class="flex-1" @click="emit('update:modelValue', false)">Cancelar</NxButton>
        <NxButton class="flex-1" :loading="isSaving" @click="submit">Guardar</NxButton>
      </div>
    </template>
  </NxModal>
</template>
