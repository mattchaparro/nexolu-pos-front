  ...
  <div class="grid grid-cols-4 gap-2 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8">
    ...
    >
      <i
        ...
        :class="openSaleByTable.has(table.id) ? 'text-amber-500' : 'text-slate-300'"
      />
      <p
        class="text-[11px] font-bold leading-tight"
        :class="openSaleByTable.has(table.id) ? 'text-amber-900' : 'text-slate-500'"
      >
        {{ table.name }}
      </p>
      <p v-if="openSaleByTable.has(table.id)" class="text-[10px] font-semibold leading-tight text-amber-700">
